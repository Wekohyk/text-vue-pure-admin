...
let overList: { index: number }[] = [];
// 存放第一个选中的元素和最后一个选中元素，只能存放这两个元素
let selectedList: { index: number | string }[] | { item: any; index: any }[] =
  [];
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    // 鼠标移入
    const setCurrentValue = (index: number) => {
    ...
    // 鼠标点击
    const selectValue = (index: string | number, item: any) => {
      if (props.disabled) return;
      ...
      if (len < 2) {
        ...
        if (len === 1) {
          // 顺时针排序
          if (selectedDirection === 'right') {
            if ('item' in selectedList[0] && 'item' in selectedList[1]) {
              console.log('123', selectedList[0].item, selectedList[1].item);
              emit('selectedVal', {
            ...
            if ('item' in selectedList[0] && 'item' in selectedList[1]) {
              console.log('456', selectedList[1].item, selectedList[0].item);
              emit('selectedVal', {
